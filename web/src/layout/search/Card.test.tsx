...
const saveScrollPositionMock = jest.fn();
...
describe('Card', () => {
  ...
  describe('Render', () => {
    it('renders component', () => {
      ...
      const repoLink = screen.getByRole('button', { name: 'Repository link' });
      expect(repoLink).toHaveProperty('href', 'https://github.com/artifacthub/hub');

      const statsLink = screen.getByRole('button', { name: 'Dev stats link' });
      ...
      // expect(globalScores[1]).toHaveStyle({ borderColor: 'var(--rm-green)' });
